...
const argv = yargs(process.argv.slice(2))
    ...
    .option('keyrange', { describe: 'Number of unique keys to cycle through', type: 'number', default: 100 })
...
class CircularBuffer {
    constructor(size = 100) { // Reduced size from 1000
...
async function runWorker(workerCfg) {
    ...
    const pool = genericPool.createPool(factory, {
        max: 5, // Reduced pool size to prevent memory issues
        min: 2,
    ...
    const latencies = new CircularBuffer(100);
    let finished = false;
    let batchId = 0;
    const startTime = performance.now();
    
    // Smaller key pool to reduce memory usage
    const keyPool = [];
    const actualKeyRange = Math.min(workerCfg.keyRange, 100); // Limit key range
    for (let i = 0; i < actualKeyRange; i++) {
    ...
    async function populateKeys() {
        ...
        try {
            ...
            for (let i = 0; i < Math.min(20, actualKeyRange); i++) { // Reduced from 100
                const key = keyPool[i];
    ...
    async function doBatch() {
        if (consecutiveErrors > 5) { // Reduced from 10
        ...
        try {
            const pipeline = client.pipeline();
            const pipelineSize = Math.min(randInt(workerCfg.pipelineRange), 20); // Limit pipeline size
    ...
    async function mainLoop() {
        ...
        const concurrency = Math.min(5, Math.ceil(workerCfg.numConnections / 10)); // Reduced and adaptive concurrency
        const workers = [];
            ...
            workers.push((async () => {
                ...
                while (!finished) {
                    try {
                        ...
                        const backoffMs = Math.min(100, 10 * Math.pow(2, Math.min(consecutiveErrors, 3)));
    ...
    setInterval(() => {
        ...
        // Periodically clear latency buffer and force garbage collection
        if (Math.random() < 0.5) { // More frequent clearing
            latencies.clear();
            if (global.gc) {
                global.gc();
            }
...
if (isMainThread) {
    ...
    const memoryCheck = setInterval(() => {
        ...
        if (memMB > 1000) { // If using more than 1GB
            console.warn(`WARNING: High memory usage: ${memMB}MB`);
            if (global.gc) {
                global.gc();
            }
        }
